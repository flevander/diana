...
object AggrChromBuilder {
	def apply(aggr:Int, mean:Boolean) = new AggrChromBuilder(aggr, mean)
}

class AggrChromBuilder(val aggr: Int, val mean:Boolean = false, sizeIncrement: Double = 2.0) extends ChromBuilder(sizeIncrement) {
