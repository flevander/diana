...
class SwathPCEvaluator(
		var state:SwathChromatogramState,
	...
	def nullRatioProb(
			group:PCGroup,
			gv:GroupValidation,
			stats:SwathChromatogramState.Stats,
			mode:String
	):Double = {
		...
		if (mode == "rank")
			...
			throw new Exception("Unknown SwathPCEvaluation.nullRatioProb() mode '"+mode+"'")
		
		Ratios.iterate(stats.correlations.length, (ri, ui, di) => 
				covSum += SwathUtil.estimateCov(stats.correlations(ui)(di), stats.variance)
			)
		...
		*/
		...
		return 1 - chi.cumulativeProbability(SwathUtil.fishers(pvalues) / c)
	...
	def pvalueForFragments(g:PCGroup, intensities:Seq[Array[Double]]):PCGroup = {
		...
		g.pEstimates = 1 - chi.cumulativeProbability(SwathUtil.fishers(g.fragmentPvalues))
